    ...
    public void loop() {
        ...
        if(!autoEndTele){
            if(firstTeleMatchLoop){
                ...
                if(!teleTimer.isExpired() && teleTimer.getTimeElapsed()>notificationTimes[0] && !notificationsPushed[0]){
                    gamepad1.runRumbleEffect(endgame);
                    gamepad2.runRumbleEffect(endgame);
                    notificationsPushed[0] = true;
                }
                if(!teleTimer.isExpired() && teleTimer.getTimeElapsed()>notificationTimes[1] && !notificationsPushed[1]){
